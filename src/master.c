...
int master(char *prologFile, char *batchFile, char *epilogFile,
  ...
  while ((batch = readCmd(dataFp, WORK_STR_LENGTH)) != NULL) {
    ...
    MPI_Send(&jobInfo, 1, jobInfoType, slaveRank, CMD_TAG, MPI_COMM_WORLD);
    MPI_Send(batch, jobInfo.scriptSize, MPI_CHAR, slaveRank,
...
char *readCmd(FILE *stream, int initLength) {
  ...
  /* read the input stream line by line, terminate when SEPARATOR is read */
