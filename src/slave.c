...
int slave(int verbose) {
  ...
  for (;;) {
    ...
    MPI_Recv(&jobInfo, 1, jobInfoType, 0, CMD_TAG, MPI_COMM_WORLD, &status);
    if (verbose)
	...
	return EXIT_FAILURE;
      ...
      free(batch);
      /* notify the master that batch job completed, and more work can be
	 sent */
      JobExitInfo jobExitInfo = {jobInfo.jobId, WEXITSTATUS(exitStatus)};
      MPI_Send(&jobExitInfo, 1, jobExitInfoType, 0, CMD_TAG, MPI_COMM_WORLD);
      if (verbose)
