...
sub new {
    my $pkg = shift(@_);
    my $self = bless {completed => Set::Scalar->new(),
...
# ----------------------------------------------------------------------------
# method that parses a log file represented by an opened file handle, only
# lines containing 'completed' or 'failed' are taken into account
# ----------------------------------------------------------------------------
sub parse_file {
    my ($self, $fh) = @_;
    while (my $line = <$fh>) {
